...
ppc64le, checks dracut and the named kernel tree, and invokes dracut without a shell. It uses fixed
`--include`, `--install`, and `--force-drivers` arguments to install
`assets/dracut/iso-chain-launch.sh`, `iso-chain-launch.service`, and `iso-chain.target`, plus the
shell, `ip`, `curl`, required libraries, systemd support, and virtio networking. It includes no DHCP
client and publishes without overwriting. Preparation is tested with dracut 107-8.fc43 in an
ephemeral VM session and does not modify the VM tooling repository.
...
The dedicated `iso-chain.target` is selected with `rd.systemd.unit` and does not depend on a real
root device. Its required one-shot service runs the launcher exactly once after
`systemd-udev-settle.service`, enters `emergency.target` on failure, and remains active on success;
it neither falls through to dracut root discovery nor repeats the probe. The launcher inventories
non-loopback interfaces and compares normalized sysfs MAC values. Exactly one match proceeds; zero
or multiple matches emit a fixed `adapter-match: failed` marker before the service fails. No
...
Unit tests fault the manifest parser, canonicalization, complete command-line boundary, GRUB
generation, dracut invocation, service/target installation, ISO inspection, no-overwrite behavior,
QEMU argument construction, evidence parsing, and fixed-error redaction. Shell tests run the
launcher with fake sysfs and command boundaries for unique, missing, and duplicate adapters;
